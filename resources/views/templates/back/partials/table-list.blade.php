                        ...
                        <div class="col-sm-6 table-commands">
                            @if($tableListData['enable_lines_choice'])
                                ...
                                <span class="input-group lines">
                                        ...
                                        <input id="input_lines" class="form-control" type="number" name="lines"
                                               value="{{ $tableListData['lines'] }}"
                                               placeholder="{{ trans('global.table_list.placeholder.lines') }}">
                                    </span>
                            @endif
                        </div>
                        ...
                        <div class="col-sm-6 table-commands text-right">
                            @if(!empty($tableListData['search_config']))
                                <span class="input-group search">
                                            <span class="input-group-addon" for="input_search"><i
                                                        class="fa fa-search"></i></span>
                                            <input id="input_search" class="form-control" type="text" name="search"
                                                   value="{{ $tableListData['search'] }}"
                                                   placeholder="{{ trans('global.table_list.placeholder.search') }}">
                                    @if($tableListData['search'])
                                        <span class="input-group-addon"><a
                                                    href="{{ route($tableListData['routes']['index'], ['search' => null, 'lines' => $tableListData['lines']]) }}"><i
        ...
        @foreach($tableListData['columns'] as $column)
            <th>
                @if(isset($column['sort_by']))
                    <a href="{{ route($tableListData['routes']['index'], ['search' => $tableListData['search'], 'lines' => $tableListData['lines'], 'sort-by' => $column['sort_by'], 'sort-dir' => !$tableListData['sort_dir']]) }}"
                       class="spin-on-click">
                        @if($tableListData['sort_by'] === $column['sort_by'] && $tableListData['sort_dir']) <i
                                class="fa fa-sort-asc"></i>
                        @elseif($tableListData['sort_by'] === $column['sort_by'] && !$tableListData['sort_dir']) <i
        ...
        <tr>
                ...
                <td>
                    ...
                    @elseif(isset($column['activate']) && !empty($column['activate']))
                        <div class="swipe-group">
                            <input class="swipe" id="activate_{{ $entity->id }}" type="checkbox"
                                   name="{{ $entity->id }}" @if($entity->getAttribute($column['key'])) checked @endif>
                            <label class="swipe-btn activate" data-url="{{ route($column['activate']) }}"
                                   data-id="{{ $entity->id }}" for="activate_{{ $entity->id }}"></label>
                        </div>
                        {{-- show image --}}
            ...
            <td class="actions text-right">
                ...
                <form role="form" class="form-inline" method="GET"
                      action="{{ route($tableListData['routes']['edit'], $entity->id) }}">
                    <a href="#" class="text-info submit-form spin-on-click"><i class="fa fa-pencil-square"
                                                                               title="{{ trans('global.action.edit') }}"></i></a>
                </form>

                <form role="form" class="form-inline" method="POST"
                      action="{{ route($tableListData['routes']['destroy']) }}">
                    <input type="hidden" name="_method" value="DELETE">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="_id" value="{{ $entity->id }}">
                    <a href="#" class="text-danger confirm"
                       data-confirm="@foreach($confirm['attributes'] as $attribute){{ $entity->getAttribute($attribute) }} @endforeach"><i
                                class="fa fa-trash" title="{{ trans('global.action.delete') }}"></i></a>
                </form>
            </td>
        </tr>
    @endforeach
        ...
        <tr>
            <td colspan="{{ sizeof($tableListData['columns']) + 1 }}" class="text-center">
                <span class="text-info"><i
                            class="fa fa-info-circle"></i></span> {{ trans('global.table_list.results.empty') }}
            </td>
        </tr>
    @endif
    </tbody>
            ...
            <div class="row">
                ...
                <div class="col-sm-4 table-commands">
                    <a href="{{ route($tableListData['routes']['create']) }}">
                        <button class="btn btn-success spin-on-click"><i
                                    class="fa fa-plus-circle"></i> {{ trans('global.action.add') }}</button>
                    </a>
                </div>
